...
import { getInfiniteTalkTaskStatus, downloadAndUploadInfiniteTalkVideo } from '@/lib/infinitetalk/client'
...
interface InfiniteTalkMetadata {
  imageUrl?: string
  audioUrl?: string
...
/**
 * InfiniteTalk Process API - Polls prediction status and updates generations
 *
 * Flow:
 * 1. Find generations with status 'processing' that have a prediction_id
 * 2. Call getInfiniteTalkTaskStatus(prediction_id) for each
...
export async function POST() {
  try {
    ...
    // Get processing OR pending generations for this user that have a prediction_id
    ...
    // Filter to only those with prediction_id
    const generationsWithPredictionId = (processingGenerations || []).filter(gen => {
      const metadata = gen.metadata as InfiniteTalkMetadata | null
      return metadata?.prediction_id
    })

    if (generationsWithPredictionId.length === 0) {
      return NextResponse.json({
        success: true,
    ...
    for (const generation of generationsWithPredictionId) {
      const metadata = generation.metadata as InfiniteTalkMetadata
      const predictionId = metadata.prediction_id!


      try {
        // Get task status from WaveSpeed API
        const statusResult = await getInfiniteTalkTaskStatus(predictionId)
        ...
        if (task.status === 'completed' && task.videoUrl) {
          ...
          const uploadResult = await downloadAndUploadInfiniteTalkVideo(
            task.videoUrl,
            generation.user_id,
            generation.id,
            adminClient
          ...
          await adminClient
            ...
            .update({
              ...
              metadata: {
                ...
                error: task.error || 'InfiniteTalk generation failed',
              } as unknown as Json,
            })
            .eq('id', generation.id)

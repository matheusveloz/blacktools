...
import { getTaskStatus, getVideoUrl, downloadAndUploadVideo } from '@/lib/sora2/client'
...
/**
 * Sora 2 Process API - Polls task status and updates generations
 *
 * Flow (Async API):
...
export async function POST() {
  try {
    ...
    // Include 'pending' in case the status update failed after task creation
    ...
    if (generationsWithoutTaskId.length > 0) {
      // Log for debugging (generations without task_id are expected in some cases)
      logger.debug('[Sora2 Process] Generations without task_id:', generationsWithoutTaskId.map(g => ({ id: g.id, status: g.status })))
    }
        ...
        if (task.status === 'completed') {
          ...
          if (externalVideoUrl) {
              ...
              const { error: completeError } = await adminClient
                ...
                .update({
                  ...
                  metadata: {
                    ...
                    original_url: externalVideoUrl // Keep original URL for reference
                  }
                })
                .eq('id', generation.id)

              if (completeError) {
                // Update failed - report as still processing so polling continues
                results.push({
                  id: generation.id,
                  task_id: taskId,
                  status: 'processing', // Still processing since DB update failed
                  progress: 100,
            ...
            await refundCredits(
              ...
              generation.credits_used, // Refund all to subscription credits (default)
              0,
          ...
          // Still in progress (submitted or in_progress) - update progress
